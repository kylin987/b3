...
class labelMod extends commonMod {
	...
	public function get_category($where,$data) {
		...
		return $this->model->table('category','A')->where($condition.$langwhere)->cache($where['cache'])->limit($where['limit'])->order('sequence '.$sequence.' , ' .$where['order'])->select();
	}
    ...
    public function get_content($where,$data) {
        ...
        $loop="
            SELECT {$expand_field}A.*,B.name as cname,B.subname as csubname,B.mid
             ...
             {$position}
             {$expand}
             WHERE {$condition}{$langwhere}{$related} ORDER BY {$where['order']} LIMIT {$where['limit']}
            ";
    ...
    public function get_tags($where,$data)
    {
        ...
        $langwhere='A.lang='.model('lang')->langid().' ';
