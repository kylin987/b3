...
class tagsModel extends commonModel
{
    ...
    {
        $lang=model('lang')->current_lang();
        return $this->model->table('tags','A')->where('lang='.$lang['id'].$where)->count();
    }
    ...
    {
        $lang=model('lang')->current_lang();
        return $this->model->table('tags_category')->where('lang='.$lang['id'])->order('cid asc')->select();
    }
