...
class contentModel extends commonModel
{
	...
	//
	public function add_cid($aid,$cid){
		$cid=intval($cid);
		$info_data=$this->model->table('content')->where('aid='.$aid)->find();
		$info_data['aid']='';
		$info_data['cid']=$cid;
		$info_data['urltitle']=$info_data['urltitle'].time();
		$cont_aid=$this->model->table('content')->data($info_data)->insert();
		$cont_data=$this->model->table('content_data')->where('aid='.$aid)->find();
		$cont_data['id']='';
		$cont_data['aid']=$cont_aid;
		$this->model->table('content_data')->data($cont_data)->insert();
		$expand_data=$this->model->table('category')->where('cid='.$cid)->find();
		if($expand_data['expand']!=0){
		$expand_data=$this->model->table('expand_model')->where('mid='.$expand_data['expand'])->find();
		$table=$expand_data['table'];
		$expand_data=$this->model->table('expand_content_'.$table)->where('aid='.$aid)->find();
		$expand_data['id']='';
		$expand_data['aid']=$cont_aid;
		return $this->model->table('expand_content_'.$table)->data($expand_data)->insert();
		}
