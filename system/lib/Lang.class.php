...
class Lang
{
	...
	static public function init($config=array(),$cookie_pre='')
	{
		...
		//定义当前语言
		if(!defined(__LANG__))
       	    define('__LANG__',self::getLang(self::$config['LANG_DEFAULT'],$cookie_pre));//定义当前选择的语言 
		self::$config['LANG_PACK_PATH']=self::$config['LANG_PACK_PATH'].__LANG__.'/';	//设置当前语言的语言包目录
	}


    //获取新配置文件
    static public function langCon() {
        require (__ROOTDIR__ . '/lang/'.__LANG__.'/config.php');
        return $config;
    }
	...
	//获取当前语言
    static private function getLang($default='zh',$cookie_pre)
    {		

		if(!self::$config['LANG_OPEN']){
			$lang = 'zh';
		}
			...
			else if(isset($_COOKIE[$cookie_pre.'cp_language']))//获取用户上次选择的语言
 			{
				$lang=$_COOKIE[$cookie_pre.'cp_language'];
			}
			else
			{
				$lang=$default;//默认语言
			...
			setcookie($cookie_pre.'cp_language',$lang,time()+3600*24*365,'/');		
			return $lang;
    }
