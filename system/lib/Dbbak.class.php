 ...
 */
     public function exportSql($table='',$subsection=0,$ext_sql='')
	 {
     	...
     	if($subsection == 0)
		{
			...
			$this->transfer=$this->transfer.$ext_sql;
		...
		else
		{
     		...
     		for($i=0;$i<count($sqlArry);$i++)
			{
     		...
     		if(!empty($ext_sql)){
     			$fileName = $this->dataDir.date("Ymd",time()).'_part1000.sql.php';
     			$this->_writeSql($fileName,$ext_sql);
     		}
     	}
     	return true;
    }
...
*/
    public function importSql($dir='',$search='',$replace=''){
		
		if(is_file($dir))
		{
			return $this->_importSqlFile($dir,$search,$replace);
		}
		...
		if($link = opendir($dir))
		{
			...
			for($i=0;$i<$num;$i++)
			{
				...
				{
					if(false==$this->_importSqlFile($dir.$fileArry[$i],$search,$replace))
					{
						return false;
					}
				}
...
//执行sql文件，恢复数据库
    protected function _importSqlFile($filename='',$search='',$replace='')
	{
		$sqls=file_get_contents($filename);
		if(!empty($search)){
			$sqls=str_replace($search, $replace, $sqls);
		}
		$sqls=substr($sqls,13);
		$sqls=explode("\n",$sqls);
		if(empty($sqls))
