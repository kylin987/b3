...
class Plugin
{
	...
	//运行插件	
    static public function run($plugin_name,$action_name,$data=NULL,$return=false)
	{
		...
		if($return){
			return self::$_instance[$plugin_name]->$action_name($data);
		}else{
			self::$_instance[$plugin_name]->$action_name($data);
		}
		return true;
	}	
