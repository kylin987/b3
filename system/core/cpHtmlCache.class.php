...
class cpHtmlCache {
	...
	static public function refresh($cachePath,$url){
		$file = self::file($cachePath,$url);
		if(is_file($file)){
			@unlink($file);
		}
	...
	static public function file($cachePath,$url){
		//第一个字符是'/'，则去掉
		if ($url[0] == '/') {
			$url = substr($url, 1);
		}
		require (__ROOTDIR__.'/inc/config.php');
		if($config['MOBILE_OPEN']&&$config['MOBILE_DOMAIN']==$_SERVER["HTTP_HOST"]){
			$mobile='m/';
		}
		if( empty($url) ){ //首页
			$file = 'index.html';
